...
export const har2mmd = (
  ...
  for (const e of entries) {
    if (
      !target_hosts.includes(e['host']) ||
      TYPES_TO_IGNORE.includes(e['content_mime'])
    ) {
      continue
    }
    console.log(e)
  ...
  if (config['compact']) {
    ...
    for (const e of entries) {
      ...
      if (
        crr &&
        crr['host'] == e['host'] &&
        crr['content_mime'] == e['content_mime'] &&
        crr['status'] == e['status'] &&
        e['content_mime'].startsWith('image/')
      ) {
        crr['repeat']++
      } else {
        viewEntries.push(e)
  ...
  for (const e of viewEntries) {
    ...
    if (e['repeat'] && e['repeat'] > 1) {
      ...
      if (
        notes.includes('params') &&
        Object.keys(e['query_params']).length > 0
      ) {
        if (notes.includes('value')) {
          mmd.push(
            '   Note right of visitor: [params]<br/>' +
              kvList(e['query_params']).join('<br/>')
          )
        } else {
          mmd.push(
            '   Note right of visitor: [params]<br/>' +
              Object.keys(e['query_params']).join('<br/>')
          )
        }
      }
      if (notes.includes('params') && Object.keys(e['post_data']).length > 0) {
        if (notes.includes('value')) {
          mmd.push(
            '   Note right of visitor: [post]<br/>' +
              kvList(e['post_data']).join('<br/>')
          )
        } else {
          mmd.push(
            '   Note right of visitor: [post]<br/>' +
              Object.keys(e['post_data']).join('<br/>')
          )
        }
      }
      if (
        notes.includes('send_cookies') &&
        Object.keys(e['send_cookies']).length > 0
      ) {
        if (notes.includes('value')) {
          mmd.push(
            '   Note right of visitor: [send-cookie]<br/>' +
              kvList(e['send_cookies']).join('<br/>')
          )
        } else {
          mmd.push(
            '   Note right of visitor: [send-cookie]<br/>' +
              Object.keys(e['send_cookies']).join('<br/>')
          )
        }
      }
      mmd.push(
      ...
      if (
        notes.includes('set_cookies') &&
        Object.keys(e['set_cookies']).length > 0
      ) {
        if (notes.includes('value')) {
          mmd.push(
            '   Note left of ' +
          ...
          mmd.push(
            '   Note left of ' +
              hosts[e['host']] +
              ': [set-cookie]<br/>' +
              Object.keys(e['set_cookies']).join('<br/>')
          )
        }
      }
    }
